...
<div class="space-y-0.5 mt-1">
  ...
  <div class="text-[#6e7681]">&nbsp;[help]&nbsp; Show menu &nbsp;·&nbsp; [clear]&nbsp; Clear &nbsp;·&nbsp; [exit]&nbsp; Quit</div>
</div>`;
  }
  ...
  async function runCommand(raw: string) {
    ...
    if (cmd === "1" || cmd === "send a message" || cmd === "message" || cmd === "mail" || cmd === "email") {
    ...
    } else if (cmd === "2" || cmd === "view resume" || cmd === "resume" || cmd === "cv") {
      push({ kind: "output", html: `<span class="text-green">▶ Navigating to About section...</span>` });
      await wait(500);
      push({ kind: "output", html: `<span class="text-[#8b949e]">→ Scroll down for the <span class="text-green">Download Resume</span> button.</span>` });
      await wait(300);
      ...
      push({ kind: "output", html: `<span class="text-green">akhilesh-maurya</span> <span class="text-[#8b949e]">— DevOps Engineer | SRE | Cloud Architect</span>` });

    } else if (cmd === "pwd") {
      push({ kind: "output", html: `<span class="text-[#8b949e]">/home/akhilesh/portfolio</span>` });

    } else if (cmd === "ls" || cmd === "ls -la") {
      push({ kind: "output", html: `<span class="text-cyan">about/&nbsp;&nbsp;skills/&nbsp;&nbsp;experience/&nbsp;&nbsp;resume.pdf&nbsp;&nbsp;contact.sh</span>` });

    } else if (cmd === "date") {
      push({ kind: "output", html: `<span class="text-[#8b949e]">${new Date().toString()}</span>` });

    } else if (cmd === "uptime") {
      push({ kind: "output", html: `<span class="text-[#8b949e]">akhilesh has been up for 8+ years — no downtime recorded ✓</span>` });

    } else if (cmd === "cat resume.pdf") {
      push({ kind: "output", html: `<span class="text-green">▶ Downloading resume...</span>` });
      await wait(400);
      const a = document.createElement("a");
      a.href = "/Akhilesh_CV.pdf";
      a.download = "Akhilesh_Maurya_Resume.pdf";
      a.click();
      ...
      push({ kind: "system", html: `<span class="text-green">$ </span><span class="term-cursor"></span>` });
      setBusy(false);
      return;
  ...
  function handleKeyDown(e: KeyboardEvent<HTMLInputElement>) {
    if (e.key === "Enter" && !busy) {
      const val = input; setInput(""); runCommand(val); return;
    }
    if (e.key === "ArrowUp") {
      e.preventDefault();
  ...
  return (
    <section id="contact" className="py-24 relative z-10">
      <div className="max-w-[1100px] mx-auto px-10" ref={ref}>
        ...
        {/* Heading */}
        <motion.div className="text-center mb-16" variants={fadeUp} custom={0} initial="hidden" animate={inView ? "visible" : "hidden"}>
          <h2 className="font-sans text-[2.6rem] font-bold text-text1 mb-3">
            <span className="font-mono text-base text-green mr-3">04.</span>
            Get In Touch
          </h2>
          <p className="text-[0.95rem] text-text2 max-w-[440px] mx-auto leading-[1.7]">
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-1.5 px-4 py-3 bg-bg3 border-b border-[rgba(48,54,61,0.8)]">
              ...
              <span className="font-mono text-xs text-text3 ml-2 flex-1">ssh akhilesh@portfolio.dev</span>
              <span className="font-mono text-xs text-text3">interactive shell</span>
            </div>

            {/* Output area */}
            <div
              ref={bodyRef}
              className="px-5 pt-4 pb-2 font-mono text-[0.81rem] leading-[1.85] h-[300px] overflow-y-auto"
            ...
            >
              {history.map((entry, i) => {
                ...
                if (entry.kind === "error")
                  return <div key={i} className="block text-[#ff6b6b]">{entry.text}</div>;
                return null;
              })}
            </div>

            {/* Input row */}
            <div className="flex items-center gap-1 px-5 py-3 border-t border-[rgba(48,54,61,0.5)]">
              <span className="text-green font-mono text-[0.81rem] flex-shrink-0">$ </span>
              <input
                ref={inputRef}
                type="text"
                value={input}
                ...
                className="flex-1 bg-transparent border-none outline-none font-mono text-[0.81rem]
            ...
            <div className="px-5 py-1.5 border-t border-[rgba(48,54,61,0.3)] flex gap-3 flex-wrap items-center">
              {["1", "2", "3", "help", "clear"].map((hint) => (
                <button
                  key={hint}
                  onClick={() => { setInput(hint); inputRef.current?.focus(); }}
                  className="font-mono text-[0.65rem] text-text3 hover:text-cyan transition-colors cursor-pointer bg-transparent border-none"
                >
                  [{hint}]
                </button>
              ))}
              <span className="font-mono text-[0.65rem] text-text3 ml-auto">↑↓ history · Tab complete</span>
            </div>
          </div>
        </motion.div>

        {/* Social links — 3 only: LinkedIn, Email, Phone (revealed on click) */}
        <motion.div
        ...
        >
          ...
          <a
            ...
            className="flex items-center gap-2 text-[0.88rem] text-text2 hover:text-text1 transition-colors duration-200"
          >
            <Linkedin size={18} /> LinkedIn
          </a>

          {/* Email — click to reveal address */}
          <button
            onClick={() => setEmailRevealed((v) => !v)}
            className="flex items-center gap-2 text-[0.88rem] text-text2 hover:text-text1 transition-colors duration-200 bg-transparent border-none cursor-pointer relative"
            ...
            <AnimatePresence mode="wait">
              {emailRevealed ? (
                <motion.span
                  ...
                  className="overflow-hidden whitespace-nowrap font-mono text-green"
                >
                  {CONTACT.email}
                </motion.span>
              ) : (
          ...
          {/* Phone — icon only, click to reveal number */}
          <button
